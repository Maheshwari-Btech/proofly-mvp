...
import { ReadinessSkeleton } from '../common/Skeleton';
...
export const ReadinessView: React.FC<ReadinessViewProps> = ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {opportunities.map((opp) => {
              const score = assessments[opp.id]?.readinessScore;
              return (
                <option key={opp.id} value={opp.id}>
                  {opp.company} &mdash; {opp.title} {score !== undefined ? `(${score}%)` : '(Not analyzed)'}
                </option>
              );
            })}
          </select>
          ...
          <button
            ...
            className="p-2 text-slate-500 hover:text-purple-700 hover:bg-purple-50 rounded-xl border border-slate-200 transition-colors flex items-center gap-1 text-xs font-semibold"
          >
      ...
      {/* When opportunity has not been analyzed yet */}
      {!currentAssessment ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-8 sm:p-12 text-center shadow-xs">
          <Sparkles className="h-12 w-12 text-purple-600 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-slate-900 mb-2">Ready to Analyze this Target Role</h2>
          <p className="text-slate-600 text-sm max-w-lg mx-auto mb-6">
            Compare your {evidence.length} evidence artifact{evidence.length === 1 ? '' : 's'} against the {activeOpp.requirements.length} skill requirements for {activeOpp.title} at {activeOpp.company}.
          </p>
          <button
            onClick={() => onRecalculateAssessment(activeOpp.id)}
            className="bg-purple-600 hover:bg-purple-700 text-white font-bold px-6 py-3 rounded-xl text-sm transition-all shadow-sm inline-flex items-center gap-2"
          >
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-xs">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              ...
              <div className="lg:col-span-4 flex flex-col items-center justify-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <CircularProgress
                  score={currentAssessment.readinessScore}
                  size={140}
                  strokeWidth={3.8}
                ...
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-700 leading-relaxed flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {currentAssessment.summaryAnalysis ||
                      'Your technical foundation shows solid component design skills.'}
                  </div>
                </div>
              </div>
            </div>
