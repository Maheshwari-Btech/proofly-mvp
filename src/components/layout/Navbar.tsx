        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-6">
            ...
            >
              ...
              <div>
                <span className="text-2xl font-extrabold text-purple-700 tracking-tight block leading-tight">
                  Proofly
                </span>
            ...
            {session.isAuthenticated ? (
              <div className="relative">
                ...
                >
                  <div className="h-9 w-9 rounded-full bg-purple-100 border border-purple-200 flex items-center justify-center text-purple-700 font-bold text-xs shadow-2xs">
                    {profile.avatarInitials || 'CM'}
                  </div>
                  <div className="text-left hidden 2xl:block">
                    <p className="text-xs font-bold text-slate-800 leading-none">{profile.fullName}</p>
                    <p className="text-[10px] text-slate-500 truncate max-w-[110px] leading-tight mt-0.5">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-1.5 text-xs font-bold text-slate-700 hover:text-purple-700 hover:bg-slate-100 rounded-xl transition-colors"
                >
                  Sign In
                </button>
                <button
                  onClick={() => onOpenAuthModal('register')}
                  className="px-4 py-1.5 text-xs font-bold text-white bg-purple-600 hover:bg-purple-700 rounded-xl transition-all shadow-xs flex items-center gap-1.5"
                >
          ...
          <div className="flex xl:hidden items-center gap-2">
            ...
            <button
              onClick={() => (session.isAuthenticated ? onSelectTab('profile') : onOpenAuthModal('login'))}
        ...
        <div className="xl:hidden bg-white border-b border-slate-200 px-4 pt-2 pb-6 space-y-1 shadow-lg animate-in slide-in-from-top-4 duration-150">
          <div className="px-3 py-2 mb-2 bg-purple-50 rounded-xl flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-purple-700">{profile.targetRole || profile.email}</p>
            </div>
            {session.isAuthenticated ? (
              <span className="px-2 py-0.5 text-xs font-bold bg-white text-emerald-700 rounded-full border border-emerald-200">
                Logged In
              </span>
            ) : (
              <button
                ...
                className="px-2.5 py-1 text-xs font-bold bg-purple-700 text-white rounded-lg"
              >
                Sign In
              </button>
            )}
