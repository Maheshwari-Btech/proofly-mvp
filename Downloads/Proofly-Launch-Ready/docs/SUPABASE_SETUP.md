...
supabase db push
```
*Or via the Supabase Dashboard SQL Editor:* Paste and execute each `.sql` file in sequence (`001` -> `002` -> `003` -> `004` -> `005`).
...
| `skill_swap_peers` | `SELECT` | `authenticated` | `status = 'available'` and `skill_swap_active = true` |
...
1. Create/configure a Supabase project and enable Email/Password authentication.
2. Add your exact production origin under **Authentication → URL Configuration → Site URL** and Redirect URLs.
3. Copy `.env.example` to `.env` and set `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, and the server-only `GEMINI_API_KEY`.
4. Apply migrations in order with `supabase db push`. The old 2024 legacy migration is intentionally not part of this release because it defined a conflicting schema.
5. If a Supabase project already contains the old 2024 schema, do **not** blindly run the new migrations on it. Use a fresh project or perform a reviewed database migration first.
6. Run `npm run lint` and `npm run build`.
7. Run the production server with `npm start`; it respects the hosting provider's `PORT` environment variable.
8. Keep `GEMINI_API_KEY` server-side only. Never place it in a `VITE_*` variable.
9. Verify `/api/health`, sign-up/sign-in, email confirmation, creating an opportunity, adding evidence, readiness recalculation, Career Trial submission, SkillSwap, mentor request, contact form, and data export before launch.
