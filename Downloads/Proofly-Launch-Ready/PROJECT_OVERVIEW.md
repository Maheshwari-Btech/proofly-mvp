...
### A. Frontend Architecture (React 19 + Vite + Tailwind CSS)
...
3. **`opportunity_requirements`**: Competency requirements broken down by technical category, tools, and importance tiers.
4. **`evidence_items`**: Verified projects, certificates, GitHub repos, courses, and workplace credentials with verification metrics.
...
8. **`skill_progress_records`**: User skill progression history.
9. **`skill_swap_peers`**: Reciprocal peer learning profiles and availability.
...
- SkillSwap discovery exposes only the dedicated peer-directory fields; private profile rows are not used as a public directory.
...
This ZIP contains the launch-hardening release. The old 2024 legacy migration was removed because it conflicted with the current 2026 schema. See `docs/LAUNCH_AUDIT.md` for the audit and `docs/SUPABASE_SETUP.md` for the deployment checklist.
